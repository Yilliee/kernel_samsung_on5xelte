			      ...
			      struct wireless_dev *wdev, const void *data, int len)
{
	...
	ret = slsi_mib_encode_bool(&mib_data, SLSI_PSID_UNIFI_MAC_ADDRESS_RANDOMISATION_ACTIVATED, 1, 0);
...
static int slsi_rtt_set_config(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int len)
{
	...
	u16 rtt_peer;
	u16 vif_idx = 0;
	u16 center_freq0, center_freq1, channel_freq, width;
...
void slsi_rx_range_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	...
	if (!nlattr_start) {
		SLSI_ERR(sdev, "failed to start NL80211_ATTR_VENDOR_DATA\n");
		kfree_skb(nl_skb);
		goto exit;
	}
...
void slsi_rx_range_done_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	...
	SLSI_MUTEX_LOCK(ndev_vif->vif_mutex);
	nl_skb = cfg80211_vendor_event_alloc(sdev->wiphy, NLMSG_DEFAULT_SIZE,
					     SLSI_NL80211_RTT_COMPLETE_EVENT, GFP_KERNEL);
#ifdef CONFIG_SCSC_WLAN_DEBUG
	SLSI_DBG1_NODEV(SLSI_GSCAN, "Event: %s(%d)\n",
